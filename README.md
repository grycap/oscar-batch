...
## Configuration file
The OSCAR-Batch coordinator can be easily configured by providing a JSON file like the [config.json](https://github.com/grycap/oscar-batch/blob/main/config.json). It collects the info regarding the MinIO object storage system (including endpoint, access credentials, bucket name, and folder), the OSCAR cluster (also involving the endpoint and the access credentials), and the name of the service to be executed.

Notice that the repo provides a small script to generate the config file programmatically. See [gen-config-file.py](https://github.com/grycap/oscar-batch/blob/main/gen-config-file.py).

For more documentation regarding OSCAR, visit [https://grycap.github.io/oscar/](https://docs.oscar.grycap.net). For more details on how the coordinator works, refer to the code in this repo.
